...
async function conductInterview() {
  ...
  const finalFeedbackPrompt = ChatPromptTemplate.fromMessages([
    ...
    [
      "system",
      `Based on the entire interview conversation:
      
  1. Give a brief general feedback on the candidate’s **strengths** and **areas of improvement** with the heading strength and area of improvements to specify.
  
  2. Create a table summarizing:
     | Question(short) | User's Answer (Short Summary) | Feedback |
     Use simple words like "introduction", "tech stack", "experience" for the question column.
  
  Return only the feedback and table.`,
    ],
  ]);
